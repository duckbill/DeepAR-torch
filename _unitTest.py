...
import argparse
from data_process.util import create_dataset,unpadding,scale_raw, prep_data, Params, set_logger,save_checkpoint,load_checkpoint,save_dict_to_json,plot_all_epoch
...
def train(model: nn.Module,
    ...
    for i, (train_batch, idx, labels_batch) in enumerate(tqdm(train_loader)):
        ...
        for t in range(params.train_window):
            ...
            mu, sigma, hidden, cell = model(x, idx, hidden, cell)
...
def train_and_evaluate(model: nn.Module,
    ...
    for epoch in range(params.num_epochs):
        ...
        plot_all_epoch(ND_summary[:epoch + 1], args.dataset + '_ND', params.plot_dir)
        plot_all_epoch(loss_summary[:(epoch + 1) * train_len], args.dataset + '_loss', params.plot_dir)
    ...
    if args.save_best:
        ...
        f.close()
        plot_all_epoch(ND_summary, print_params + '_ND', location=params.plot_dir)
        plot_all_epoch(loss_summary, print_params + '_loss', location=params.plot_dir)
