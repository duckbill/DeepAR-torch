    ...
    def fit_epoch(self, train_loader, test_loader, epoch):
        ...
        for i, (train_batch, idx, labels_batch) in enumerate(tqdm(train_loader)):
            ...
            for t in range(self.params.train_window):
                ...
                mu, sigma, hidden, cell = self(x, idx, hidden, cell)
    ...
    def evaluate(self, test_loader, plot_num, sample=True):
        ...
        with torch.no_grad():
            ...
            for i, (test_batch, id_batch, v, labels) in enumerate(tqdm(test_loader)):
                ...
                input_mu = torch.zeros(batch_size, self.params.test_predict_start, device=self.params.device) # scaled
                input_sigma = torch.zeros(batch_size, self.params.test_predict_start, device=self.params.device) # scaled
                hidden = self.init_hidden(batch_size)
                cell = self.init_cell(batch_size)
                ...
                for t in range(self.params.test_predict_start):
                    ...
                    mu, sigma, hidden, cell = self(test_batch[t].unsqueeze(0), id_batch, hidden, cell)
                    input_mu[:,t] = v_batch[:, 0] * mu + v_batch[:, 1]
                    input_sigma[:,t] = v_batch[:, 0] * sigma

                if sample:
                    samples, sample_mu, sample_sigma = self.test(test_batch, v_batch, id_batch, hidden, cell, sampling=True)
                    raw_metrics = update_metrics(raw_metrics, input_mu, input_sigma, sample_mu, labels, self.params.test_predict_start, samples, relative = self.params.relative_metrics)
                else:
                    sample_mu, sample_sigma = self.test(test_batch, v_batch, id_batch, hidden, cell)
                    raw_metrics = update_metrics(raw_metrics, input_mu, input_sigma, sample_mu, labels, self.params.test_predict_start, relative = self.params.relative_metrics)

                if i == plot_batch:
                    if sample:
                        sample_metrics = get_metrics(sample_mu, labels, self.params.test_predict_start, samples, relative = self.params.relative_metrics)
                    else:
                        sample_metrics = get_metrics(sample_mu, labels, self.params.test_predict_start, relative = self.params.relative_metrics)                
                    ...
                    plot_eight_windows(self.params.plot_dir, plot_mu, plot_sigma, label_plot, self.params.test_window, self.params.test_predict_start, plot_num, plot_metrics, sample)
